...
df_train = pd.read_csv('../data/train_data.csv', parse_dates=['tradeTime'])
df_test = pd.read_csv('../data/test_a.csv', parse_dates=['tradeTime'])
...
def get_fea(df):
    ...
    # data = df[df['newWorkers'] != 0].reset_index(drop=True)
    # data['tradeTime_month'] = data['tradeTime'].apply(lambda x: x.month)
    # newWorkers = data[['plate', 'tradeTime_month', 'newWorkers']].drop_duplicates().sort_values(
    #     by=['plate', 'tradeTime_month', 'newWorkers']).reset_index(drop=True)
    # newWorkers = newWorkers['newWorkers'].groupby(newWorkers['plate']).mean().to_dict()
    # df['newWorkers'] = df.apply(lambda x: newWorkers[x.plate] if (x.newWorkers == 0) else x.newWorkers, axis=1)
    ...
    # plate_community_nums = df['communityName'].groupby(df['plate']).nunique().reset_index().rename(
    #     columns={'communityName': 'plate_community_nums'})
    # print(plate_community_nums)
    # df = pd.merge(df, plate_community_nums, on='plate', how='left')
    ...
    # df = pd.merge(df, area_money_communityName, on='communityName', how='left')
    ...
    # df = pd.merge(df, area_money_plate_month, on=['plate', 'tradeTime_month'], how='left')
    # df = pd.merge(df, area_money_plate_month_max, on=['plate', 'tradeTime_month'], how='left')
    # df = pd.merge(df, area_money_plate_month_min, on=['plate', 'tradeTime_month'], how='left')
    # df = pd.merge(df, area_money_plate_month_std, on=['plate', 'tradeTime_month'], how='left')
    # print(df[['area_money_plate_month', 'area_money_plate_month_max', 'area_money_plate_month_min', 'area_money_plate_month_max']])

    # df = pd.merge(df, area_money_region, on='region', how='left')

    # df = pd.merge(df, community_cast, on='communityName', how='left')
    # df = pd.merge(df, area_cast, on='int_area', how='left')
    ...
    # 每个plate小区数目
    #
    #     # plate_feas = ['area_money']
    #     #
    #     # for fea in tqdm(plate_feas):
    #     #     grouped_df = df.groupby('plate').agg({fea: ['min', 'max', 'mean', 'sum', 'median']})
    #     #     grouped_df.columns = ['plate' + '_'.join(col).strip() for col in grouped_df.columns.values]
    #     #     grouped_df = grouped_df.reset_index()
    #     #     df = pd.merge(df, grouped_df, on='plate', how='left')
...
def get_result(train, test, label, my_model, need_sca=True, splits_nums=5):
    ...
    for index, (train_index, test_index) in enumerate(k_fold.split(train)):
        ...
        # X_test = xgb.DMatrix(X_test)  , ntree_limit=model.best_iteration
    ...
    if not os.path.exists('re_train.csv'):
        re_train.to_csv('re_train.csv', index=None)
    return re_sub, oof
...
def filter_train(train):
    re_train = pd.read_csv('re_train.csv')
    re_train = pd.merge(re_train, train, on='ID', how='left')
    re_train['diff'] = re_train['tradeMoney'] - re_train['score']
    re_train['abs_diff'] = re_train['diff'].apply(lambda x: abs(x))
    re_train = re_train.sort_values(by=['abs_diff'], ascending=False).reset_index(drop=True)
    # print(re_train[['ID', 'tradeMoney', 'abs_diff']])
    re_train['diff_rate'] = re_train['abs_diff'] / (re_train['tradeMoney'])
    re_train = re_train.sort_values(by=['diff_rate'], ascending=False).reset_index(drop=True)
    # re_train = re_train[re_train['abs_diff'] >= 3000]
    re_train = re_train[re_train['diff_rate'] >= 0.6]
    print(re_train)
    print(re_train.shape)
    train = train[~train['ID'].isin(list(re_train['ID']))]
    return train
...
if not os.path.exists('re_train.csv'):
    get_result(X, test_data, y, xgb_model_bs, need_sca=True, splits_nums=5)
    ...
    sub[['tradeMoney']].to_csv('../result/sub_xgb_filter.csv', index=None, header=False)
...
# 去除tradeNewMeanPrice、tradeMeanPrice为0
# train = train[(train['tradeNewMeanPrice'] != 0) & (train['tradeMeanPrice'] != 0)].reset_index(drop=True)
# test = test[(test['tradeNewMeanPrice'] != 0) & (test['tradeMeanPrice'] != 0)].reset_index(drop=True)
#
# sub = test[['ID']]
# X = train[features]
# y = train['tradeMoney']
# test_data = test[features]
# sub1, oof1 = get_result(X, test_data, y, xgb_model_bs, need_sca=False, splits_nums=5)
#
# print(sub.shape, sub1.shape)
#
# sub = pd.concat((sub, sub1)).reset_index(drop=True)
# sub = sub.drop_duplicates(['ID'], keep='first').reset_index(drop=True)
# print(sub.shape)
...
# 0.9186981186663093  0.9191320050052451  0.9182990245666364  0.918887700891674  0.9189613011274232  0.9190691350521775


# 0.9221010543233273    0.01
